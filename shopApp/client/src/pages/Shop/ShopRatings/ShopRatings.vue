    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <span class="block positive" @click="setType(2)" :class="{active:selectType==2}">
            全部<span class="count">{{ratings.length}}</span>
          </span>
          <span class="block positive" @click="setType(0)" :class="{active:selectType==0}">
            满意<span class="count">{{positiveNum}}</span>
          </span>
          <span class="block negative" @click="setType(1)" :class="{active:selectType==1}">
            不满意<span class="count">{{ratings.length-positiveNum}}</span>
          </span>
        </div>
        <div class="switch" :class="{on:onlyShowText}">
          <span class="iconfont icon-check_circle"  @click="showText()"></span>
...
export default { 
    data() {
        return {
            selectType:2, // 默认选择全部评价
            onlyShowText:true,// 默认只显示有内容的评价
        }
    },
    mounted() {
    ...
    computed: {
        ...
        // 过滤评价
        filterRatings(){
            const {selectType,ratings,onlyShowText} = this
            return  ratings.filter( rating => {
                // 条件
                //  selectType 全部2 满意0 不满意1
                //  rating.rateType 满意0 不满意1
                //  onlyShowText 有内容1 全部内容0 rating.text.length
